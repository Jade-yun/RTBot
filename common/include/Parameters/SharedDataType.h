...
public:
    T buffer[MAX_CMD_QUEUE_LEN];

    bool push(const T &cmd)
    {
        uint32_t h = head.load(std::memory_order_relaxed);
        uint32_t t = tail.load(std::memory_order_acquire);
        if ((h - t) >= MAX_CMD_QUEUE_LEN)
        {
            return false; // 队列满
        }
        buffer[h % MAX_CMD_QUEUE_LEN] = cmd;
    ...
    bool pop(T &out)
    {
        ...
        out = buffer[t % MAX_CMD_QUEUE_LEN];
    ...
    size_t pop_batch(T *out, size_t count)
    {
        ...
        {
            out[i] = buffer[(t + i) % MAX_CMD_QUEUE_LEN];
        }
...
{
    SPSC_CmdQueue<HighLevelCommand> cmd_queue;
