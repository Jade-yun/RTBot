...
ControlMode axis_mode[AXIS_NUM] = {
    ControlMode::CSP, // 轴0使用位置模式
    ControlMode::CST  // 轴1使用转矩模式
};
...
void updateStatus(double position, double velocity) {
...
int main() {
    ...
    while (true) {
        ...
        for (int i = 0; i < AXIS_NUM; ++i) {
            ...
            if (axis_mode[i] == ControlMode::CSP) {
                position[i] += 10; // 每周期增加10
                ec_iface.writeTargetPosition(i, position[i]);
            } else if (axis_mode[i] == ControlMode::CST) {
